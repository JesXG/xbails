   ...
   function injectNativeNode(type, message) {
    ...
    return nodeContent.filter(item => {
        if (item.tag === 'biz' && Array.isArray(item.content)) {
            const hasInteractiveNativeFlow = item.content.some(
                tag => tag.tag === 'interactive' && tag.attrs?.type === 'native_flow' && tag.attrs?.v === '1'
            );
            return !hasInteractiveNativeFlow; 
        }
        return true; 
    });
};
    ...
    const emilia = new kurumi(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = emilia.detectType(content);
   
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await emilia.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await emilia.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await emilia.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        const albumContent = await emilia.handleAlbum(content, jid, quoted)
                        return albumContent;
                        
                    case 'EVENT':
                        return await emilia.handleEvent(content, jid, quoted)
                        
                    case 'POLL_RESULT':
                        return await emilia.handlePollResult(content, jid, quoted)
                }
            }
